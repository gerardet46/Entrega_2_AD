...
## LO D'ENTREGAR
```{r, include=FALSE}
#Empram la neteja de dades propocionada al git de la classe per poder unificar totes les preguntes en un sol document.
...
```{r}
correlació_esp_veu=cor(df$esport,df$veu, method = "pearson")
correlació_ali_scor=cor(df$alimentacio,df$veu, method = "pearson")
...
```{r}
taula_esport=addmargins(table(df$esport,df$veu))
taula_alimentacio=addmargins(table(df$alimentacio,df$veu))
#Ara extreurem les frequ marginals de les files
freq_abs_esport_f=taula_esport[-dim(taula_esport)[1],"Sum"]
freq_abs_alimentacio_f=taula_alimentacio[-dim(taula_alimentacio)[1],"Sum"]
#També ho farem de les columnes
freq_abs_esport_c=taula_esport["Sum",-dim(taula_esport)[2]]
#(Siran igual amb l'alimentació)
#Ara calcularem la taula de freqüències esperades
freq_esperades_esport=freq_abs_esport_f%*%t(freq_abs_esport_c)/length(df$esport)
freq_esperades_alimentacio=freq_abs_alimentacio_f%*%t(freq_abs_esport_c)/length(df$alimentacio)
freq_esperades_esport
freq_esperades_alimentacio
taula_alimentacio
taula_esport
```

Notem que no, llavors hauríem d'agrupar els nivells per a que es cumpleixi la condició necessària:
